    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.transactions.update", [$transaction->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('saver') ? 'is-invalid' : '' }}" name="saver_id" id="saver_id" required>
                    @foreach($savers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('saver_id') ? old('saver_id') : $transaction->saver->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('saver'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('payment_method') ? 'is-invalid' : '' }}" name="payment_method_id" id="payment_method_id" required>
                    @foreach($payment_methods as $id => $entry)
                        <option value="{{ $id }}" {{ (old('payment_method_id') ? old('payment_method_id') : $transaction->payment_method->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('shares_quantity') ? 'is-invalid' : '' }}" type="number" name="shares_quantity" id="shares_quantity" value="{{ old('shares_quantity', $transaction->shares_quantity) }}" step="1">
